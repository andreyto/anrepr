...
This vignette demonstrates how you would initiate a report generation in your R code
by creating the `anrep` class and use its methods for adding both structural and data
...
complex analysis pipeline with a graphical reporting facility. 
...
elements if your code creates those R objects which implement the 
...
Popular R frameworks like built-in Sweave and [RMarkdown](https://cran.r-project.org/package=rmarkdown) / 
...
languages such as HTML or Markdown. 
When the document is "rendered" by Knitr, the code chunks written in a language such
as R are replaced by the results of their evaluation. 
A [large ecosystem of tools](https://cran.r-project.org/web/views/ReproducibleResearch.html) 
have been developed in the R community for formatting various analysis objects into the markup 
representation.
...
programming language such as R or Python. As the analysis proceeds through multiple stages such as data loading, 
data cleaning and the application of the same or different computations to the subsets of data, we often need to:
...
- Make both the reporting process and the analysis fully automated, so that it is easy to present the updated results to our
  research collaborators in frequent iterative discussions
- Have the report in a format that can be easily archived and viewed in another location by the collaborators without any need for an additional 
  backend infrastructure such as the database or Web application servers
- Keep the effort required to either add or modify the reporting instructions for **both structure and content** of the report very low compared 
  to the effort that we are willing to dedicate to developing the analysis itself. As a reasanoble trade off, we are willing to tolerate an
  auto-generated document layout that will not be as polished as a layout coming from a hand-tuned document template.
...
fit well all other applications.
...
In our work, we have adopted the approach taken by the `pander` package that provided a report generator class `Pandoc`. Our `anrep` class
...
objects, and maintaining consistency of the numbering even in the presence of error conditions during execution of the analysis code.
The `pander::Pandoc` does not provide any facilities for structuring the report beyond a generic ability for inserting  
header tags. 
...
The fundamental distinction of our work from `Nozzle` is that our `anrep` class (as well as the `pander::Pandoc` class) emits Markdown text 
...
Package-level functions in `pander` can be used to automatically convert a multitude of R data types into Markdown with a single
call. That includes extraction of vector and raster images from plot objects produced by the major R graphing libraries. 
Using a well-known external `Pandoc` converter utility, our report in the Markdown format is automatically converted into any of multiple final 
presentation formats, such as HTML pages and slide shows, or Word and PDF documents. New style sheet file and JavaScript libraries can be supplied 
to completely change the appearance and behaviour of the rendered HTML pages. Our Markdown reports can be also embedded into 
'Knitr' documents or even loaded as active content into a Shiny application. Users can add their own arbitrary Markdown text into 
the report. Markdown is very easy to write considering the simplicity of its syntax relative to, for example, HTML.
...
```{r hello1, results="asis", warning = FALSE, message = FALSE, cache=FALSE}
...
```{r hello1, echo=FALSE, message = FALSE, warning = FALSE, results='markup', comment=''}
```

##Note on running under `Knitr`
In the `anrep` constructor above, we have passed the `out.formats="knitr"` argument.
This was done because we used [Knitr](https://cran.r-project.org/package=knitr)
to generate this vignette directly from the code chunks inserted into a hand-written
Markdown document.
`out.formats="knitr"` option tells the `anrep` report object to stream its own 
generated Markdown to the standard output. `Knitr` collects the output from 
the code chunk and treats it as a ready-to-render Markdown because we marked the 
corresponding Knitr chunk as having the result of type `asis`:
...
The `knitr` output mode also turns off the subreport (linked multiple documents) 
generation and collects everything into a single Markdown document. Links to
saved data files will not work if `devtools::build_vignettes()` call was used
to generate a vignette for the R package.

Note that we have used a couple of `pander` package options to turn off some
auto-magic in `pander`. This was done to avoid a potential interference with 
rendering in `Knitr` when processing collated output from multiple calls to `pander` 
functions. `anrep` internally collates the output from multiple `pander` calls.

This vignette document was built with `knitr` for demonstrating the `anrep` API. The default intended mode
for the `anrep`, however, is to let the `anrep$save` method call `Pandoc` conversion utility directly 
and generate a static Web site with multiple Web pages and data files, while running 
outside of the `Knitr` environment. 

This mode would have been used automatically if we just used `report = anrep("Hello World")`,
or `report = anrep("Hello World",out.formats="html")`. This is how you should normally call the 
`anrep` constructor when instrumenting your pipeline.

You will be able to view such full auto-generated Web site following the link 
...
After defining the functions above, we run the report generation pipeline expressed by these functions. In this vignette code,
we told the pipeline *not* to export the generated Markdown code into HTML, in order to avoid
dependency on the external Pandoc executable on the system where this vignette code might be checked.

```{r cache=FALSE, results="hide", warning = FALSE, message = FALSE}
make_example_sections_report("example_sections_report",export=F)
```

During actual use on your own system where you installed Pandoc, you would instead use the following call to produce 
the output Web site:
