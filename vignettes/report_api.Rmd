...
by creating the `anrep` class and using its methods for adding both structural and data
...
This code adds to the report two pairs of tables and plots in a loop.
...
for(i in c(1,5)) {
  report$add.table(mtcars[i:(i+5),],
                   caption=sprintf("Hello Table for rows %s-%s",i,i+5))
  report$add(with(mtcars[i:(i+5),],plot(mpg,hp)),
             caption=sprintf("Hello Plot for rows %s-%s",i,i+5),graph.unify=T)
}
# Printing the return value from the save() method will show
# rendered Markdown if running under Knitr:
report$save()
```
...
to be passed for rendering `asis` (see the corresponding value for Knitr chunk 
option `result`). This is why we had to use `cat` when we actually wanted to see 
...
the link provided for a large example script further in this document. For now, we will 
use one more Knitr-rendered example for the purposes of introducing `anrep` 
API features.

You can use other output formats supported by `Pandoc` as values for the `out.formats` argument,
but not all types of elements inserted into the generated Markdown are supported by every possible 
...
report$add.header("Second header with subsections") %anrep>>% { 
  ...
  report$add.descr("*Computing things here. Complex computations.*", caption="Starting complex computes")
  fit = lm(mpg~hp,mtcars)
  report$add.descr("*Having done computing. Took a while.*", caption="Finishing complex computes")
...
below, we told the pipeline *not* to export the generated Markdown code into HTML, in order to avoid
dependency on the external Pandoc executable on the system where this vignette code might be automatically 
checked (e.g. CRAN servers).
...
the output Web site from our example pipeline:
