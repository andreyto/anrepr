...
make_example_sections_report <- function(report_dir=NULL,export=T) {
  ...
  # function uses out `report` object to add data to the report as well as
  ...
  my.func.3<-function(subreports.header) {
    ...
    report$add.header(subreports.header) %anrep//% {
      ...
      report$add.header("H2.1.subreport 1") %anrep//% {
        ...
        # Generate a unique file name into we could save some data inside the report directory
        report$add.descr(paste("Generated a unique file name to save some extra output:",
                               report$make.file.name("data.csv")))
      }
      # The closing } above has returned us to the reporting level that was current
  ...
  my.func.2<-function() {
    ...
    report$add.header("H2.1") %anrep>>% {
      ...
      # result in three subreports, each with two sub-subreports itself
      for(i in 1:3) {
